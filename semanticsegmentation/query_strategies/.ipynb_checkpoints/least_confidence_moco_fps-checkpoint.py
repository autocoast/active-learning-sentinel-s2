...
class LeastConfidenceMoCoFPS(Strategy):
    def __init__(self, dataset, net):
        super(LeastConfidenceMoCoFPS, self).__init__(dataset, net)
    ...
    def query(self, n, cfg):
        ...
        labeled_idxs, _ = self.dataset.get_labeled_data()
        ...
        top32 = unlabeled_idxs[sorted_indices[:N_UNCERTAINTY]]

        if N_UNCERTAINTY == n:
            return top32

        initial_indices = (labeled_idxs.tolist() + top32.tolist())


        data_pool = self.embeddings[unlabeled_idxs[sorted_indices[N_UNCERTAINTY:]]]
        
        diverse_samples = farthest_point_sampling(data_pool, num_samples=(n - N_UNCERTAINTY))

        diverse_samples = np.array(diverse_samples) + N_UNCERTAINTY

        final_selection = top32.tolist() + sorted_indices[diverse_samples.tolist()].tolist()
